...
## Phase 1: Verification Subagents (parallel, `sonnet`)
...
**IMPORTANT:** Save your results to `{OUTPUT_DIR}/verification-{BATCH_NUMBER}.md` using the Write tool.
```

## Phase 2: Calibrator (single agent, `opus`)
...
**IMPORTANT:** Save your full verdicts to `{OUTPUT_DIR}/calibration.md` using the Write tool.
```
