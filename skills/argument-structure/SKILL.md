...
description: >
  Restructure a draft or idea with the Minto Pyramid Principle: extract the one-sentence
  answer, 2-4 MECE arguments and the evidence under each, diagnose the gaps, and deliver
...
This is how the skill delivers. You render the pyramid + plan as a single HTML file saved to the user's output folder, then share the link in chat with a 2-4 line summary. Do not paste the full analysis into the chat.

**File location:** Save the HTML to whatever folder output files are shared with the user in the current environment. In Claude Code, that is typically the current working directory (or a designated outputs directory if the project has one). In other environments, use the convention appropriate to that platform.
...
```
[View your Minto pyramid](./minto-pyramid-{slug}.html)
