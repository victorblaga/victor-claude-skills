...
A single top-tier Calibrator agent (highest effort available) reads all dimension findings files, dedupes cross-agent overlap, and assigns a **blast radius** per surviving finding. Its output drives the Phase 4 / Phase 5 bucket split. This is the highest-leverage reasoning step in the sweep — it gates what gets applied without review — so it gets the strongest model, regardless of what tier the dimension agents ran at.
...
Launch at top tier, highest effort. Pass the findings files and the scope as input.
...
**IMPORTANT:** Save to `{OUTPUT_DIR}/calibration.md` via Write. Then report counts back to the orchestrator.
```
