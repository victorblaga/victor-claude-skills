...
**IMPORTANT:** Save your findings to {OUTPUT_FILE} using the Write tool. Do not write to any other location.

Read files and run searches in parallel when independent. You may spawn up to 3 explorer subagents to trace cross-file usage; explorers are mid tier, medium effort.
```
...
Use the Agent tool at mid tier with high effort for each. Launch all 4 in a single message to run in parallel:

```
Agent(description="Duplication & types", prompt="<Agent A prompt with substitutions>")
Agent(description="Dead & legacy code", prompt="<Agent B prompt>")
Agent(description="Weak types & cycles", prompt="<Agent C prompt>")
Agent(description="Guards & comments", prompt="<Agent D prompt>")
```
