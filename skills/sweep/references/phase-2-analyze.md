...
Launch 8 parallel `opus` dimension agents via the Agent tool in a **single message** so they run concurrently. Each agent:
...
**IMPORTANT:** Save your findings to `{OUTPUT_DIR}/findings/<your-dimension>.md` using the Write tool. Do not write to any other location.
```
...
Spawn Explore subagents if you need to trace how candidates are used across the codebase. Do not spawn more than 3.
```
...
Use the Agent tool with `model: "opus"` for each. Launch all 8 in a single message to run in parallel. Example pattern:

```
Agent(description="Duplication finder", model="opus", prompt="<Agent 1 prompt with substitutions>")
Agent(description="Type consolidation", model="opus", prompt="<Agent 2 prompt>")
