...
- **Effort**: If the harness exposes an effort control, run all dimension agents and the Calibrator at the highest tier. Cruft detection requires semantic judgment, not checklist matching.
- **Parallel subagents**: Launch all 8 dimension agents simultaneously in a single turn. The orchestrator must fan out deliberately.
