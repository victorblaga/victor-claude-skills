...
- **Auto** — a top-tier Adjudicator subagent renders verdicts. Activate when: the user says `auto` (e.g. `/sweep auto`), the sweep runs inside a goal loop, or the harness signals non-interactive execution. Announce the mode at Phase 1.
...
Express tiers by relative capability, not memorized model names — lineups change. "Top tier" = strongest available (Opus/Fable-class); "mid tier" = Sonnet-class; "small tier" = Haiku-class.

| Role | Tier / effort | Rationale |
|------|---------------|-----------|
| 4 dimension agents (Phase 2) | mid tier, high effort | Scoped research with judgment calls — "is this cruft or intentional?" Modern mid-tier models handle a two-dimension brief in one context. |
| Calibrator (Phase 3) | top tier, highest effort | Blast-radius judgment gates auto-apply; the single highest-leverage reasoning step. |
| Adjudicator (Phase 5, auto mode only) | top tier, highest effort | Renders accept/defer verdicts unsupervised; needs fresh, maximally capable context. |
| Applier (Phase 4) | mid or small tier, low effort | Mechanical per-file application of pre-analyzed findings, with explicit veto rule (kick back to triage if subtler than judged). |
