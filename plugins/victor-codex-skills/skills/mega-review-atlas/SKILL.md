...
- **Subagents:** use `spawn_agent` for the Step 2 explorers (or explore sequentially if subagents are unavailable — preserve the same `inventory/` output files).
- **Capability mapping:** flagship → **Sol**; mid → **Terra**; small → **Luna**. Explorers run at mid or better; boundary judgment and the freeze checklist stay in the orchestrator at flagship.
...
Read repository instructions (`AGENTS.md` / `CLAUDE.md` and what they reference). Resolve target, entry points, repo root, source mode. Create the output directory; record the current HEAD SHA. Tell the user: this builds scope, not review findings.
