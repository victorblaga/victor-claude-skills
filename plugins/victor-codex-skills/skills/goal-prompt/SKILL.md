...
Default target is **Codex** unless the user asks for Claude Code. For Codex prompts, refer to `$victor-codex-skills:workstream-implementer`, `$simplify`, and `$victor-codex-skills:mega-review`. For Claude prompts, refer to `/workstream-implementer`, `/simplify`, and `/mega-review`.
