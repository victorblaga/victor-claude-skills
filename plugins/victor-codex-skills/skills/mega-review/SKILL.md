...
- To use explorer subagents when it needs to trace code across files
...
**How to search:** Read files, use Grep and Glob to explore. Spawn explorer subagents when you need to trace patterns across many files.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/code-quality.md`.
```
...
**How to search:** Read files, use Grep and Glob to explore. Spawn explorer subagents to trace service interactions, message flows, and error propagation paths.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/architecture.md`.
```
...
**How to search:** Read files, use Grep and Glob. Spawn explorer subagents to trace data flow end-to-end through pipeline stages.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/correctness.md`.
```
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/test-quality.md`.
```
...
**How to search:** Read files, Grep for exception handling patterns, error logging, SQL queries. Spawn explorer subagents to trace error propagation paths.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/security-error-handling.md`.
```
...
**How to search:** Read the new files, then deliberately read EXISTING comparable files (other pipelines, other test suites, other services) for comparison. Use Grep to find patterns across the codebase. Spawn explorer subagents to survey how things are done elsewhere.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/pattern-conformity.md`.
```
...
**How to search:** Read the new files, then read the EXISTING code that covers similar ground. Use Grep and Glob extensively to find duplication and parallel implementations. Spawn explorer subagents to map out shared code usage across pipelines.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/refactoring-opportunities.md`.
```
...
**How to search:** Read files, use Grep and Glob to find loops, collection operations, database calls. Spawn explorer subagents to trace data flow and identify where collections are processed.
...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/performance.md`.
```
...
Write your verification results to `{OUTPUT_DIR}/verification-{BATCH_NUMBER}.md`.
```
...
**Read the actual code** when the verification results are ambiguous or when you need more context to calibrate severity. Use Grep, Glob, and Read tools. Spawn explorer subagents if needed.
...
**IMPORTANT:** After completing your analysis, write your full verdicts to `{OUTPUT_DIR}/skeptic.md`.
```
...
**Read the actual code** to understand each tension. Use Grep, Glob, and Read tools. Spawn explorer subagents to trace how the architectural assumption plays out across the codebase.
