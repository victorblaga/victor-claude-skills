...
Launch all applicable dimension subagents via `spawn_agent` **in a single message** so they run concurrently. If subagents are unavailable, run dimensions sequentially yourself while preserving the same output structure. Capability class and `reasoning_effort` come from `{OUTPUT_DIR}/review-plan.md` (default: flagship / high for dimension agents).
