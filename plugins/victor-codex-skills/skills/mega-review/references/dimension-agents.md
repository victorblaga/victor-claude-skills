...
**IMPORTANT:** After completing your analysis, write your full findings to `{OUTPUT_DIR}/{OUTPUT_FILE}`. Do not write anywhere else.
```
...
Spawn explorer subagents to trace service interactions, message flows, and error propagation paths.
```
...
Spawn explorer subagents to trace data flow end-to-end.
```
...
Spawn explorer subagents to trace error propagation paths.
```
...
Spawn explorer subagents to survey how things are done elsewhere in the codebase.
```
...
Use searches extensively to find duplication and parallel implementations. Spawn explorer subagents to map shared-code usage.
```
...
Spawn explorer subagents to trace where collections are produced and consumed.
```
