...
Launch via `spawn_agent` with **flagship capability class (Sol)** and **`reasoning_effort: xhigh`**. Do **not** use `max`. Do **not** run planning in the orchestrator — a large diff read there is re-billed every subsequent turn and risks window exhaustion.
...
| Class | Codex examples | Claude Code examples | Typical use |
|-------|------------------|---------------------|-------------|
| **flagship** | Sol | Fable, Opus-class | Judgment, recall-critical review, calibration, synthesis |
| **mid** | Terra | Sonnet-class | Verification, exploration, mechanical assembly |
| **small** | Luna | Haiku-class | Pre-specified command runs only |

When model names change, map by capability — do not update the skill for every lineup change.

**Intelligence level** (`reasoning_effort` on Codex): separate lever — `xhigh`, `high`, `medium`, `low`. **`max` is never part of the default pipeline.**
