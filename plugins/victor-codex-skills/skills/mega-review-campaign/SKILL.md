...
- **Unit reviews:** each selected item runs the Codex-native `mega-review` skill in unit-packet mode; that skill spawns its own dimension agents via `spawn_agent`. Keep campaign-level execution sequential — the nested fan-out is the parallelism.
- **Capability mapping:** flagship → **Sol**; mid → **Terra**; small → **Luna**. Campaign planning, cross-unit verification, and synthesis run at Sol / `reasoning_effort: xhigh`.
