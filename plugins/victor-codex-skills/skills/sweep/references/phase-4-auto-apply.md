...
For each file with ≥1 LOW-blast finding, launch an Applier subagent on a mid- or small-tier model (Terra/Luna-class) with `reasoning_effort: low` in parallel. Bound concurrency at 8 parallel Appliers; if there are more files, batch them.
...
Launch on a mid- or small-tier model (Terra/Luna-class) with `reasoning_effort: "low"` per file. Substitute `{FILE}`, `{FINDING_IDS}`, `{FINDING_DETAILS}` (pull the full finding text for each ID from `calibration.md`), `{OUTPUT_DIR}`.
