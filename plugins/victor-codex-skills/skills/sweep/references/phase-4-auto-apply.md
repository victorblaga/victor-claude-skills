...
For each file with ≥1 LOW-blast finding, launch a subagent on a mid- or small-tier model (Terra/Luna-class) with `reasoning_effort: low` in parallel. Bound concurrency at 8 parallel Appliers; if there are more files, batch them.
